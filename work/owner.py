...
def owner_dist_p(df):
    ...
    fig.update_layout(title="Owner Distribution")
...
def owner_dist_h(df):
    ...
    fig.update_layout(title="Owner Distribution",xaxis_title="NFT Count",yaxis_title="Owner Count")
    fig.layout.template='plotly_dark'
    pio.write_image(fig,"owner_dist_h.png",width=1000,height=750)

def get_erc721_transfers(es_key,contract_addr):
...
def owner_stats(df):
    ...
    owners_cnt=len(owners[owners["VALUE"]>0])
    ...
    return (owners_cnt,supply,most_owned,mean_owners)
